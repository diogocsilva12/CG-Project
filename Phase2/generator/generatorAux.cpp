...
/**
 * Generates a circle using matrix transformations
 */
/**
...
void circle(float radius, int segments, const std::string& filename) {
    ...
    // Create a scale matrix for the radius
    Matrix4x4 scaleM = scaleMatrix(radius, 1.0f, radius);

    // Generate vertices for a unit circle in the XZ plane
    Vertex center = {0.0f, 0.0f, 0.0f};
    // Apply transformation to center (not really needed for center point, but for consistency)
    center = transformVertex(center, scaleM);
    
    for (int i = 0; i < segments; i++) {
        float angle1 = i * angleStep;
        float angle2 = (i + 1) * angleStep;
        
        // Create vertices on the unit circle
        Vertex v1 = {sin(angle1), 0.0f, cos(angle1)};
        Vertex v2 = {sin(angle2), 0.0f, cos(angle2)};
        
        // Apply scaling
        v1 = transformVertex(v1, scaleM);
        v2 = transformVertex(v2, scaleM);
        
        // Add triangle (connect center to two adjacent points on circumference)
        vertices.push_back(center);
        vertices.push_back(v1);
        vertices.push_back(v2);
    }
